...
export function initializeSqliteDatabase(options: SqliteOptions): Database {
  ...
  try {
    const lotTable = db.prepare(
      `SELECT name FROM sqlite_master WHERE type='table' AND name='project_spending_lots'`
  ...
  try {
    const lotInfo = db.prepare(`PRAGMA table_info('project_spending_lots')`).all() as Array<{
      name: string;
    }>;
